...
function Team({token}) {
  const [items,setItems]=useState(null),[credentials,setCredentials]=useState([]),[busy,setBusy]=useState(false);
  const load=()=>api('/staff',token).then(setItems); useEffect(()=>{load();},[token]); if(!items)return <Loading/>;
  const bootstrap=async()=>{setBusy(true);try{const r=await api('/staff/bootstrap-defaults',token,{method:'POST'});setCredentials(r.created||[]);await load();if(!(r.created||[]).length)alert(r.message);}finally{setBusy(false);}};
  const reset=async s=>{if(!confirm(`Reset password for ${s.name}?`))return;const r=await api(`/staff/${s.id}/reset-password`,token,{method:'POST'});setCredentials([r]);};
  return <><PageHead eyebrow="ACCESS CONTROL" title="Clinic team" copy="Separate staff accounts keep activity attributable to the correct doctor or receptionist." action={<button className="primary" onClick={bootstrap} disabled={busy}><UserCog size={17}/> {busy?'Creating…':'Create missing staff logins'}</button>}/>{credentials.length>0&&<div className="credential-panel"><div><b>Temporary credentials — copy these now</b><p>These passwords are only shown after creation/reset. Share each one privately with the correct staff member.</p></div>{credentials.map(c=><div className="credential-row" key={c.email}><span>{c.name}</span><code>{c.email}</code><code>{c.password}</code></div>)}</div>}<div className="team-grid">{items.map(s=><article className="team-card" key={s.id}><div className="avatar xl">{s.name?.[0]}</div><div className="grow"><h3>{s.name}</h3><p>{s.email}</p><span className="role-chip">{s.role}</span></div>{['doctor','receptionist'].includes(s.role)&&<button className="mini-reset" onClick={()=>reset(s)}>Reset password</button>}<div className={`active-dot ${s.active===false?'off':''}`}/></article>)}</div></>;
}
